  ...
  async createInvoice(data) {
    ...
    try {
      ...
      const [serviceRows] = await promisePool.query(
        `SELECT service_price, extra_charges, status FROM Service_Request WHERE service_request_id = ?`,
        [service_request_id]
      );
      ...
      const { service_price, extra_charges, status } = serviceRows[0];
      
      // Only allow invoices for completed services
      if (status !== 'Completed') {
        throw new Error('Invoice can only be created for completed services. Please complete the service first.');
      }
      
      // Check if invoice already exists
      const [existingInvoice] = await promisePool.query(
        'SELECT invoice_id FROM Invoice WHERE service_request_id = ?',
        [service_request_id]
      );
